			...
			if (strlen($_FILES['imagem'.$num]['name']) > 4) {
				...
				if ($Arquivo != false) {
					if (Miniatura("../../arquivos/tmp/".$Arquivo, "../../arquivos/galeria/".$codigo."/miniaturas/".$novoArquivo, 56, 56, 1, 0)) {
						...
						if (Miniatura("../../arquivos/tmp/".$Arquivo, "../../arquivos/galeria/".$codigo."/fotos/".$novoArquivo, 690, 389, 0, 1, $ArqMarcadagua, GaleriaConfigValor('marcadagua_distancia'),GaleriaConfigValor('marcadagua_posicao'),GaleriaConfigValor('marcadagua_opacidade'))) {
		...
		foreach ($Arquivos as $arquivo) {
			if (validaTipoArquivo($arquivo, 1))  { $i++;
				...
				if (Miniatura('../../arquivos/ftp/'.$pasta.'/'.$arquivo, "../../arquivos/galeria/".$codigo."/miniaturas/".$novoArquivo, 56, 56, 1, 0)) {
					...
					if (Miniatura('../../arquivos/ftp/'.$pasta.'/'.$arquivo, '../../arquivos/galeria/'.$codigo.'/fotos/'.$novoArquivo, 690, 389, 0, 1, $ArqMarcadagua, GaleriaConfigValor('marcadagua_distancia'),GaleriaConfigValor('marcadagua_posicao'),GaleriaConfigValor('marcadagua_opacidade'))) {
