...
<?
	...
	// -----------------------------------------------------------------------------------------------------------
	$busca='';
	if ($_GET['id_noticias'] > 0) {
		$busca .= ' AND tbnoticias.id_noticia='.(int)$_GET['id_noticias'];
		$dados_busca['id_noticias']= $_GET['id_noticias'];
	}
	...
	while ($tmp1 = db_lista($tmp1s)) {
		$noticias[$tmp1['titulo']." (ID - ".$tmp1['id_noticias'].")"] = $tmp1['id_noticias'];
	}
	...
	$campos = array(
		...
		array('select',		'noticias',		'id_noticias',		'250',			$noticias,			'',											''),
	...
	$SQL = "
		...
		FROM 
			tbnoticias_comentarios 
			INNER JOIN tbnoticias ON (tbnoticias.id_noticia = tbnoticias_comentarios.id_noticias)
		WHERE 1
			".$busca."
