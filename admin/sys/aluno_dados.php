...
<?
	...
	$campos = array(
		...
		array('manual',		'Cursos',	'alunoscurso',		'500',			'',					'',											''),
		...
		while ($linha = db_lista($consulta)) {
			$dados['alunoscurso'].='<label><input type=checkbox name="alunoscurso[]" value="'.$linha['id_galeria'].'"';
			if (cursoProfessor54323($dados['id_aluno'],$linha['id_galeria'])) $dados['alunoscurso'].= ' checked="checked" ';
			$dados['alunoscurso'].='> '.($linha['titulo']).'</label><br />';
		}
