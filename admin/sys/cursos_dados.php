<? 
	...
	echo "<script type=\"text/javascript\" src=\"jquery.price_format.js\"></script>";
	...
	function CarregaBairros(id_cidade, id_imovel){
		//alert(id_cidade);
		//alert(id_imovel);
		$.post('carrega_bairros.php',{id:id_cidade, id_imovel:id_imovel},
			function(data){
				$('#recebendo').html(data);
			});
	}
...
<div id="conteudo">
	...
	<? 
	$onChange="id=\"selecione\" onchange=\"CarregaBairros(this.value)\"";
	
	if ($_GET['ID']>0){
	
	$id_imovel_js=(int)$_GET['ID'];
	$onChange="id=\"selecione\" onchange=\"CarregaBairros(this.value, $id_imovel_js)\"";
...
<?
 ...
 include('../includes/Mensagem.php');
	...
	# Categoria
	$CategoriaBairros=array();
	$tmpBairross = db_consulta("SELECT * FROM tbbairros ORDER BY nome ASC");
	while ($tmpBairros = db_lista($tmpBairross)) {
		$CategoriaBairros[(htmlentities($tmpBairros['nome']))]=$tmpBairros['id_bairro'];
	}
	...
	$campos = array(
		...
		array('select',		'Cidade',	'cidade',		'150',			$Categoria3,			'<a target="_blank" style="margin:0 0 0 10px;" href="loja_categorias.php"><img width="20px" src="../img/bteditar.gif"></a>',											$onChange),
		array('select',		'Bairro',	'bairro',		'150',			'',			'', 'id="recebendo"'),
		...
		array('text',		'Valor R$',		'preco',			'100',			'',					'Ex: 1.000,00',											'id="preco"'),
	...
	if($_GET['ID']>0){	
		echo "<script>$(\"select#selecione\").trigger(\"change\");</script>";
	}
