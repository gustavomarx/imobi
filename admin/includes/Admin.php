...
function adminCampos($campos,$config,$dados) {
	...
	$saida  = '<form class="form" name="frmDados" action="../app/'.$config['arquivo'].'.php?faz=dados'.$config['urlfixo'].'" method="post" enctype="multipart/form-data">';
	...
	foreach ($campos as $campo) {
		...
		switch ($campo[0]) {
			...
			case 'password':
			case 'hidden':
				$saida .= '<input class="text" type="'.$campo[0].'" name="'.$campo[2].'" value="'.$dados[$campo[2]].'" style="width:'.$campo[3].'px;" '.$campo[6].' onfocus="this.className=\'focus\';" onblur="this.className=\'\';" />';
				break;

			case 'file':
				$saida .= '<input class="file" type="file" name="'.$campo[2].'" style="width:'.$campo[3].'px;" '.$campo[6].' />';
				...
				if (!empty($dados[$campo[2]])) {
					if (validaTipoArquivo($dados[$campo[2]],1)) {
						$saida .= '<img vspace=2 src="../../arquivos/'.$config['pasta'].'/'.$dados[$campo[2]].'" width="200px">';
...
*/
			...
			case 'textarea':
				...
				case 'textarea2':
					$saida .= '<textarea class="text-input textarea" id=""   name="'.$campo[2].'" rows="20" cols="100" '.$campo[6].' onfocus="this.className=\'focus\';" onblur="this.className=\'\';">'.$dados[$campo[2]].'</textarea>';
					break;
			...
			case 'flag':
				$saida .= '<label><input type="radio" name="'.$campo[2].'" '.$campo[6].' value="1" '.(($dados[$campo[2]]==1)?' checked="checked"':'').'> Sim </label> &nbsp; <label><input type="radio" name="'.$campo[2].'" '.$campo[6].' value="0" '.(($dados[$campo[2]]!=1)?' checked="checked"':'').'> Não </label>';
				break;

			case 'select':
				$saida .= '<select class="select" name="'.$campo[2].'" '.$campo[6].' style="width:'.$campo[3].'px;">';
				if (is_array($campo[4]))
			...
			case 'data':
				$saida .= '<input class="texto" type="'.$campo[0].'" name="'.$campo[2].'" value="'.$dados[$campo[2]].'" readonly="readonly" size="10" onfocus="this.className=\'focus\';" onblur="this.className=\'\';" '.$campo[6].' /> ';
...
function adminLista($campos,$dados,$acoes,$config,$excluir_massa=true) {
	...
	if (count($acoes)>0) $saida .= '<th style="min-width:'.$tamanhoAcoes.'px">AÇÔES</th>';
	...
	*/	
	foreach ($dados as $dado) { $i++;
		...
		if (count($acoes)>0) {
			...
			if (in_array('premium2',$acoes)) $saida .= '<a class="premium2'.(int)$dado['premium'].'" href="../app/'.$config['arquivo'].'.php?faz=premium&premium=premium&id='.$dado[$config['id']].'&origem='.urlencode(urlOrigem()).''.$config['urlfixo'].'" title="premium2">premium</a>';
		...
		foreach ($campos as $campo) {
			...
			switch ($campo[0]) {
				...
				case 'flag': // '../app/'.$FileName.".php?faz=status&id={".$cnf['id']."}&atual={flag_status}&origem=".urlencode($_SERVER['REQUEST_URI']
					...
					case 'premium': // '../app/'.$FileName.".php?faz=star&id={".$cnf['id']."}&atual={star_star}&origem=".urlencode($_SERVER['REQUEST_URI']
					$saida .= '<a class="premium2'.(int)$dado[$campo[2]].'" href="../app/'.$config['arquivo'].'.php?faz=premium&premium='.$campo[2].'&id='.$dado[$config['id']].'&origem='.urlencode(urlOrigem()).''.$config['urlfixo'].'" title="'.$campo[1].'">'.(int)$dado[$campo[2]].'</a>';
					break;
				
					case 'foto':
						$saida .= '<a href="'.$config['arquivo'].'_dados.php?ID='.$dado[$config['id']].''.$config['urlfixo'].'"><img src="../../arquivos/'.$config['arquivo'].'/'.$dado[$campo[2]].'" width="80px" height="80px" /></a>';
						break;
						
						case 'capa':
							$saida .= '<a href="'.$config['arquivo'].'_dados.php?ID='.$dado[$config['id']].''.$config['urlfixo'].'"><img src="../../arquivos/'.$config['pasta'].'/'.$dado[$campo[2]].'/capa.jpg" width="80px" height="80px" /></a>';
							break;
			
				default:
					if($campo[2]=="preco"){
						$saida .= number_format($dado[$campo[2]],2,",",".");
					}else{
						$saida .= $dado[$campo[2]];
					}
					break;
...
// * db_apagaArquivo: Apaga um arquivo cadastrado no banco de dados
// -----------------------------------------------------------------------------------------------------------
function db_apagaArquivo($nome,$config,$id) {
...
// -----------------------------------------------------------------------------------------------------------
function processaArquivo($nome,$config,$file,$tipoNomeArq=1) {
	...
	if (!empty($file[$nome]["name"])) {
		...
		if (validaTipoArquivo($file[$nome]['name'], 1) && $config[$nome]['x'] > 0 && $config[$nome]['y'] > 0 ) {
			...
			$Arquivo = FazerUpload($file[$nome],"../../arquivos/tmp/",$NovoNome);
			if ($Arquivo != false) {
				# Criando miniatura
				if ($config[$nome]['mini']['x']>0 && $config[$nome]['mini']['y']>0) {
					$miniatura2 = Miniatura("../../arquivos/tmp/".$Arquivo , "../../arquivos/".$config['pasta']."/_miniaturas/".$Arquivo, $config[$nome]['mini']['x'], $config[$nome]['mini']['y'], $config[$nome]['mini']['corte'], 0);
					if ($miniatura2) { # faz nada
					} else return false;
				}
				$miniatura = Miniatura("../../arquivos/tmp/".$Arquivo , "../../arquivos/".$config['pasta']."/".$Arquivo, $config[$nome]['x'], $config[$nome]['y'], $config[$nome]['corte'], 1);
		...
		# Arquivos
			...
			$Arquivo = FazerUpload($file[$nome],"../../arquivos/".$config['pasta']."/",$NovoNome);
...
// -----------------------------------------------------------------------------------------------------------
// * cursoProfessor: Verifica se o usuario tem permissao para acessar um certo módulo
// -----------------------------------------------------------------------------------------------------------
function cursoProfessor($usuario2,$menu2) {

	if ($usuario2==1) return true;
	if ($menu2==0) return true;

	$permitido2 = db_linhas(db_consulta("SELECT * FROM liga_prof WHERE id_galeria=".(int)$usuario2." AND id_materia=".(int)$menu2));
	if ($permitido2 > 0) {
		return true;
	} else {
		return false;
...
// * cursoProfessor: Verifica se o usuario tem permissao para acessar um certo módulo
// -----------------------------------------------------------------------------------------------------------
function cursoProfessor2($usuario22,$menu22) {

	if ($usuario22==1) return true;
	if ($menu22==0) return true;

	$permitido22 = db_linhas(db_consulta("SELECT * FROM liga_carre WHERE id_produto=".(int)$usuario22." AND id_galeria=".(int)$menu22));
	if ($permitido22 > 0) {
		return true;
	} else {
		return false;
...
// * cursoProfessor: Verifica se o usuario tem permissao para acessar um certo módulo
// -----------------------------------------------------------------------------------------------------------
function cursoProfessor333($usuario333,$menu333) {

	if ($usuario333==1) return true;
	if ($menu333==0) return true;

	$permitido333 = db_linhas(db_consulta("SELECT * FROM liga_hora WHERE id_download=".(int)$usuario333." AND id_galeria=".(int)$menu333));
	if ($permitido333 > 0) {
		return true;
	} else {
		return false;
...
// * cursoProfessor: Verifica se o usuario tem permissao para acessar um certo módulo
// -----------------------------------------------------------------------------------------------------------
function cursoProfessor54323($usuario33333,$menu33333) {

	if ($usuario33333==1) return true;
	if ($menu33333==0) return true;

	$permitido33333 = db_linhas(db_consulta("SELECT * FROM liga_aluno WHERE id_aluno=".(int)$usuario33333." AND id_galeria=".(int)$menu33333));
	if ($permitido33333 > 0) {
		return true;
	} else {
		return false;
