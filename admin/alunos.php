<? 
	...
	if ($_GET['faz']=="dados") {
		...
		if (  (! validaTipoArquivo($_FILES['imagem']['name'],1)) && (!($id_aluno>0)) ) $Erros .= "- Envie arquivo jpg |";
		...
		if ($$Config['id']>0) {
			...
			$dados['codigo']=$codigo;
			@mkdir('../../arquivos/aluno/');
			@mkdir('../../arquivos/aluno/fotos/');
			@mkdir('../../arquivos/aluno/miniaturas/');

		}
		
		# Capa
		if (strlen($Capa)>5) {
			@unlink('../../arquivos/aluno/capa.jpg');
			@rename('../../arquivos/aluno/'.$Capa,'../../arquivos/aluno/capa.jpg');
		}
	...
	if ($_GET['faz']=="fotosForm") {
		...
		for ($num=1;$num<=8;$num++) {
			if (strlen($_FILES['imagem'.$num]['name']) > 4) {
				...
				if ($Arquivo != false) {
					if (Miniatura("../../arquivos/tmp/".$Arquivo, "../../arquivos/aluno/miniaturas/".$novoArquivo, 156, 156, 1, 0)) {

						if (Miniatura("../../arquivos/tmp/".$Arquivo, "../../arquivos/aluno/fotos/".$novoArquivo, 790, 589, 0, 1, $ArqMarcadagua, alunoConfigValor('marcadagua_distancia'),alunoConfigValor('marcadagua_posicao'),alunoConfigValor('marcadagua_opacidade'))) {
	...
	if ($_GET['faz']=="fotosFtp") {
		...
		foreach ($Arquivos as $arquivo) {
			if (validaTipoArquivo($arquivo, 1))  { $i++;
				...
				if (Miniatura('../../arquivos/ftp/'.$pasta.'/'.$arquivo, "../../arquivos/aluno/miniaturas/".$novoArquivo, 156, 156, 1, 0)) {
	
					if (Miniatura('../../arquivos/ftp/'.$pasta.'/'.$arquivo, '../../arquivos/aluno/fotos/'.$novoArquivo, 790, 589, 0, 1, $ArqMarcadagua, alunoConfigValor('marcadagua_distancia'),alunoConfigValor('marcadagua_posicao'),alunoConfigValor('marcadagua_opacidade'))) {
		...
		foreach ($foto as $cod => $ativo) {
			...
			if ($ativo==1) {
				...
				if ($opcao == 'delete') {
					...
					@unlink('../../arquivos/aluno/miniaturas/'.$imagem);
					@unlink('../../arquivos/aluno/fotos/'.$imagem);
