<? 
	...
	if ($_GET['faz']=="dados") {
		...
		# Permissões
		db_consulta("DELETE FROM liga_carre WHERE id_produto=".$$Config['id']);
		foreach ($careiraaa as $valor) {
			if ($valor > 0) db_executa('liga_carre',array('id_produto'=>$$Config['id'], 'id_galeria'=>$valor));
	...
	if ($_GET['faz']=="excluir") {
		...
		if ($id>0) {
			...
			# Apagando vínculos 
			db_consulta("DELETE FROM liga_carre WHERE id_produto=".$id);
