<? 
	...
	if ($_GET['faz']=="dados") {
		...
		# Dados
		$dados = array( 'id_curso'=>$id_curso, 'mapa'=>$mapa, 'finalidade'=>$finalidade, 'titulo'=>$titulo, 'area'=>$area, 'garagens'=>$garagens, 'data'=>dataDMY_YMD($data), 'suites'=>$suites, 'quartos'=>$quartos, 'bairro'=>$bairro, 'conteudo'=>$conteudo, 'endereco'=>$endereco, 'cidade'=>$cidade, 'banho'=>$banho, 'preco'=>$preco, 'condominio'=>$condominio, 'tags'=>$tags, 'estado'=>$estado, 'cep'=>$cep );
		...
		if ($$Config['id']>0) {
			...
			header("Location: ../sys/".$Config['arquivo']."_dados.php?ID=".db_insert_id()."&msg=".urlencode('Im&oacute;vel criado com sucesso, continue o cadastro.'),true); exit;
