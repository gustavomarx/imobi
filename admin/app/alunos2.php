	...
	if ($_GET['faz']=="fotosForm") {
		...
		for ($num=1;$num<=8;$num++) {
			if (strlen($_FILES['imagem'.$num]['name']) > 4) {
				...
				if ($Arquivo != false) {
					if (Miniatura("../../arquivos/tmp/".$Arquivo, "../../arquivos/aluno/'.$codigo.'/miniaturas".$novoArquivo, 156, 156, 1, 0)) {

						if (Miniatura("../../arquivos/tmp/".$Arquivo, "../../arquivos/aluno/'.$codigo.'/fotos".$novoArquivo, 790, 589, 0, 1, $ArqMarcadagua, alunoConfigValor('marcadagua_distancia'),alunoConfigValor('marcadagua_posicao'),alunoConfigValor('marcadagua_opacidade'))) {
