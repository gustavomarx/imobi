<? 
	...
	if ($_GET['faz']=="dados") {
		...
		# Testes
		$Erros='';
		if (strlen($titulo) < 2) $Erros .= "- Insira o TITULO.";
		...
		# Formata resultados para dolar (0.00)
		//tirando pontos do formato BRA
		$preco = str_replace(".","", $preco);
		//substituindo virgula pelo ponto para formato EUA
		$preco = str_replace(",",".", $preco);	
		$dados = array( 'id_corretor'=>$id_corretor, 'id_zona'=>$id_zona, 'id_curso'=>$id_curso, 'mapa'=>$mapa, 'finalidade'=>$finalidade, 'titulo'=>$titulo, 'area'=>$area, 'garagens'=>$garagens, 'data'=>dataDMY_YMD($data), 'suites'=>$suites, 'quartos'=>$quartos, 'bairro'=>$bairro, 'conteudo'=>$conteudo, 'endereco'=>$endereco, 'cidade'=>$cidade, 'banho'=>$banho, 'preco'=>$preco, 'condominio'=>$condominio, 'tags'=>$tags, 'estado'=>$estado, 'cep'=>$cep );
		...
		if ($$Config['id']>0) {
				...
				# Permissões
				db_consulta("DELETE FROM liga_prof WHERE id_galeria=".$$Config['id']);
				foreach ($cursos as $valor) {
					if ($valor > 0) db_executa('liga_prof',array('id_galeria'=>$$Config['id'], 'id_materia'=>$valor));
				}
		...
		if ($$Config['id']>0) {
			...
			header("Location: ../sys/".$Config['arquivo']."_dados.php?ID=".$$Config['id']."&msg=".urlencode('Atualizado.'),true); exit;
	...
	if ($_GET['faz']=="excluir") {
		...
		if ($id>0) {
			...
			if (strlen($galeria['codigo'])==32) {
				...
				# Apagando vínculos 
				db_consulta("DELETE FROM liga_prof WHERE id_galeria=".$id);
	...
	// -----------------------------------------------------------------------------------------------------------
	if ($_GET['faz']=="premium") {
		list($valor) = db_dados("SELECT ".$_GET['premium']." FROM ".$Config['tabela']." WHERE ".$Config['id']."=".(int)$_GET['id']);
		if ($valor==1) $valor='0'; else $valor='1';
		
		db_executa($Config['tabela'],array($_GET['premium']=>$valor),'update', $Config['id'].'='.$_GET['id']);
	...
	if ($_GET['faz']=="fotosForm") {
		...
		for ($num=1;$num<=8;$num++) {
			if (strlen($_FILES['imagem'.$num]['name']) > 4) {
				...
				if ($Arquivo != false) {
					if (Miniatura("../../arquivos/tmp/".$Arquivo, "../../arquivos/galeria/".$codigo."/miniaturas/".$novoArquivo, 150, 150, 1, 0)) {

						if (Miniatura("../../arquivos/tmp/".$Arquivo, "../../arquivos/galeria/".$codigo."/fotos/".$novoArquivo, 800, 800, 0, 1, $ArqMarcadagua, GaleriaConfigValor('marcadagua_distancia'),GaleriaConfigValor('marcadagua_posicao'),GaleriaConfigValor('marcadagua_opacidade'))) {
	...
	if ($_GET['faz']=="fotosFtp") {
		...
		foreach ($Arquivos as $arquivo) {
			if (validaTipoArquivo($arquivo, 1))  { $i++;
				...
				if (Miniatura('../../arquivos/ftp/'.$pasta.'/'.$arquivo, "../../arquivos/galeria/".$codigo."/miniaturas/".$novoArquivo, 150, 150, 1, 0)) {
	
					if (Miniatura('../../arquivos/ftp/'.$pasta.'/'.$arquivo, '../../arquivos/galeria/'.$codigo.'/fotos/'.$novoArquivo, 800, 800, 0, 1, $ArqMarcadagua, GaleriaConfigValor('marcadagua_distancia'),GaleriaConfigValor('marcadagua_posicao'),GaleriaConfigValor('marcadagua_opacidade'))) {
