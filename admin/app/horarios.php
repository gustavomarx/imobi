...
#exit;
	...
	if ($_GET['faz']=="dados") {
		...
		# Dados
		$dados = array( 'id_categoria'=>$id_categoria, 'curso'=>$curso, 'horario'=>$horario, 'data'=>$data
		...
		# Permissões
		db_consulta("DELETE FROM liga_hora WHERE id_download=".$$Config['id']);
		foreach ($horariooo as $valor) {
			if ($valor > 0) db_executa('liga_hora',array('id_download'=>$$Config['id'], 'id_galeria'=>$valor));
	...
	if ($_GET['faz']=="excluir") {
		...
		if ($id>0) {
			...
			# Apagando vínculos 
			db_consulta("DELETE FROM liga_hora WHERE id_download=".$id);
